...
bool DNAVector::SetToSubOf(const DNAVector & v, int start, int len)
{
  ...
  if (start < 0) {
    cout << "WARNING, adjusting boundaries in DNAVector::SetToSubOf (1)";
    len += start;
    start = 0;
  }
  if (start >= v.size()){ //Need to check this first otherwise next adjustment fails
    ...
    cout << "WARNING, adjusting boundaries in DNAVector::SetToSubOf (2)";
    len = v.size() - start;
  }
...
void DNAVector::ExtendWithString(const string& extension, int extendFrom) {
  cout << "Requesting extension from: " << extendFrom << " extension size: " << extension.size();
  if(extendFrom>isize() || extendFrom<0) { 
    cout << "Requesting extension of sequence given wrong index";
    return; 
  }
...
bool vecDNAVector::SetSequence(const Coordinate& coords, DNAVector& resultSeq) const {
  if(!HasChromosome(coords.getChr())) { 
    cout << "Check Genome data! - Chromosome: " << coords.getChr() 
